...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        return (self.start, (False, False, False, False))
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nx][ny]:
                ...
                if (nx,ny) in self.corners:
                    idx = self.corners.index((nx,ny))
                    newVisited[idx] = True
                successors.append((((nx,ny),tuple(newVisited)),action,1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # Greedy sum of Manhattan distances to remaining corners
    visited = state[1]
    position = state[0]
    ...
    while remainingCorners:
        distances = [(util.manhattanDistance(current, c), c) for c in remainingCorners]
        dist, closest = min(distances)
        heuristic += dist
        current = closest
        remainingCorners.remove(closest)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # Distance between farthest pair of food dots
    maxPair = 0
